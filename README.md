...
Books under **Chanting Book 念诵本** present their readers directly after the table of contents: Daily Chants English (`daily-chants.html`), Paccayaniddeso English and Chinese (`paccayaniddeso.html`, `paccayaniddeso-chinese.html`), and all three Pāli Chanting editions (`pali-chanting-book.html`, `pali-chanting-book-chinese.html`, `pali-chanting-book-burmese.html`). **Daily Chants Burmese (`daily-chants-burmese.html`) is excluded** and retains its existing Contents/Reader behavior.

For the continuous readers, Contents and Reader header controls scroll to those positions; they must not hide one section in order to show the other. Preserve all book text and PCED functions. The chanting readers' column-label row stays in normal document flow rather than sticky positioning, so it cannot cover the first title. The first Pāli Chanting English heading must remain a paired `VANDANĀ | Homage` row.
