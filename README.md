...
The English reading panel now follows the supplied `TOW for Realization of Nibbā_na - pamc.docx` typography and layout: body and Unicode Pāli use Tahoma (Pāli italic), paragraphs retain the Word first-line and hanging-indent relationships, and headings retain the source small-cap character. The nine kamma-born kalāpas are restored as the Word source's two-column table, with aligned numbered continuations and a contained horizontal scroller on narrow screens. The Chinese panel, book text, Contents, language switching, notes, PCED terms, IDs and shared reader controls are unchanged. Structural validation confirms identical English text and reference counts before and after this formatting-only pass; rendered-browser and deployed physical-device confirmation remain pending.
