...
Chinese-Tipitaka terminology display preserves Enter/newline characters stored in the database. Shared popup v1.3.19 applies `white-space: pre-wrap` only to the standardized-terminology definition block, so paragraph and line separation appears as entered without changing database text or ordinary PCED entries. It also preserves editor-created `<strong>`/`<b>` bold formatting while escaping every other database HTML tag. The index-search popup and in-book PCED popups use the same Picture-3 typography: Georgia/Times for Latin text, the documented Chinese serif fallbacks for definitions, and Arial/Microsoft YaHei for source labels and controls; the first installed font in each device's fallback stack is used.
