...
Chinese run-level font roles are retained with Unicode source-first stacks, including Song, Kai, Hei, FangSong, Xinwei, Zhongsong, Xihei, Xingkai, YouYuan and LiSu roles. East Asian and Latin font declarations are evaluated separately, so a Chinese run that happens to inherit Sangayana as its Latin font is not misclassified as blue Pāli. The normal reading size is 19px. Sangayana-encoded Pāli and Sanskrit characters are converted to Unicode; Pāli is displayed with Times New Roman and Unicode serif fallbacks. Source alignment, first-line, hanging and side indentation are retained from both the older `left/right` and newer `start/end` Word attributes. All 312 numeric body-list paragraphs use one compact hanging-list treatment without artificial blank lines: the number itself begins at the source book's normal two-character paragraph inset, while continuation lines hang beneath the item wording. Consecutive stanza and Pāli lines likewise remain compact. Every displayed Pāli token is connected to the shared PCED lookup.
