...
The source-to-HTML font map is: 宋体 → Songti/SimSun body text; 黑体 → Heiti/SimHei; 幼圆 → YouYuan; 隶书 → LiSu; 方正魏碑_GBK → Fangzheng Weibei with Kai fallback; 华文楷体 → STKaiti/KaiTi; 华文细黑 → STXihei; 华文中宋 → STZhongsong; and Times New Roman/Times_CSX+ → Unicode serif. Sangayana-encoded Pāli is converted to Unicode and displayed with Times New Roman/Noto Serif. Source paragraph indentation is retained; detected verse and compact quotation lines have no artificial inter-line paragraph gap. The former unscoped important font override is restricted to interface chrome, and Chinese note popups retain their source font role. Source-text equality, Contents targets, table counts and inline-script syntax pass structural validation; rendered browser and deployed physical-device confirmation remain pending.
