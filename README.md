...
`samanera-sikkhapadani.html` is the complete twelve-chapter reader built from the supplied revised Word source. It is listed beside 《比库巴帝摩卡》 under `玛欣德尊者译著 Dhamma Books authored or translated by Venerable Mahinda` → `戒律 The Code of Monastic Discipline`. The opening order is the separately stored actual cover, publication details, copyright conditions, opening canonical quotation, the fixed-height scrolling Contents window, 序言、凡例, all twelve chapters, 词语汇解 and 主要参考资料.

The reader follows the source rather than adding a separate translation layer. Chapter 1 and the remaining prose chapters are Chinese-only. Chapter 2 pairs each source Pāli formula with its corresponding source Chinese pronunciation guidance and Chinese meaning in two columns; it contains no generated English translation. In Chapter 10, all four-resources reflections, the complete *Khandha paritta*, the complete *Dasadhammā sutta*, the homage dialogue, rains-residence formulas and Kaṭhina formulas are arranged as 43 source-matched Pāli–Chinese parallel rows. Printed page numbers, Roman-numeral front-matter numbers, visible source-page labels and page-turn cards are omitted from HTML. Material under one semantic heading flows continuously across former printed page turns. This no-printed-page-number and continuous-flow rule is a permanent requirement in `BOOK-READER-INTERFACE-STANDARD.md`.

Chinese run-level font roles are retained with Unicode source-first stacks, including Song, Kai, Hei, FangSong, Xinwei, Zhongsong, Xihei, Xingkai, YouYuan and LiSu roles. East Asian and Latin font declarations are evaluated separately, so a Chinese run that happens to inherit Sangayana as its Latin font is not misclassified as blue Pāli. The normal reading size is 19px. Sangayana-encoded Pāli and Sanskrit characters are converted to Unicode; Pāli is displayed with Times New Roman and Unicode serif fallbacks. Source alignment, first-line, hanging and side indentation are retained from both the older `left/right` and newer `start/end` Word attributes. All 312 numeric body-list paragraphs retain their individual Word-source indentation instead of receiving one universal hanging indent: ordinary points keep the source first-line inset and return wrapped lines to the source paragraph margin, while the smaller set of genuinely hanging or left-indented lists retains those distinct measurements. Compact point spacing remains without artificial blank lines. Consecutive stanza and Pāli lines likewise remain compact. Every displayed Pāli token is connected to the shared PCED lookup.

All 157 Word footnotes have a superscript body reference and a matching movable, non-dimming `註释` popup. The generated note-body records use the exact `data-footnote` lookup contract expected by the popup script, and validation requires every trigger ID to resolve to exactly one note body. The popup uses the approved 《止观法要》 treatment: brown `#A8734F` title bar, white title and close control, and an opaque cream reading body. Both source tables retain their cell structure in contained responsive scrollers. Twenty-six body illustrations and the two copyright/publication images are displayed in source order; the old transparent personal emblem from the Word title page is not substituted for the supplied book cover.

The reader uses the same full standard header, Book Mark, Search, A−/A+, scrolling Contents behavior, two-column parallel layouts and responsive mobile stacking as the corrected 《比库巴帝摩卡》 reader, within the current 1080px standard reader width. Static validation confirms twelve numbered chapters, 80 Contents destinations, two source tables, 28 displayed source images, 157 unique superscript references, 157 matching note records, all 1,911 meaningful source paragraphs or illustrated paragraphs represented, all 312 detected numeric body-list items normalized, 43 Chapter 10 Pāli–Chinese rows, no Chinese text inheriting the blue Pāli container style, no visible printed-page labels, no generated Hanyu Pinyin layer, no generated English translation in Chapter 2, valid fragment links, existing local assets and valid inline JavaScript. Browser visual and interaction verification remains pending because no browser executable is available in this environment. This is a manual-upload package; no push, pull request, merge or deployment was performed.

## 《比库巴帝摩卡》complete source-book reader — 22 September 2026

`bhikkhupatimokkha.html` is the complete four-chapter reader built from the supplied revised-edition Word source. Its opening sequence now follows the source and the established Dhamma Books format exactly: the actual book cover and publication details, the copyright conditions, the scrolling Contents window, then 序, 序言, 增订版序, 凡例 and the four chapters. Chapter 1 is Chinese-only in continuous book paragraphs. Chapter 2 preserves the source Pāli/Chinese matching in two columns, with no English translation, but removes all visible printed-page labels and repeated page-column headings; content under the same semantic title now flows continuously across former printed page turns. Chapters 3 and 4 retain the complete source content and are separate semantic chapter sections.
...
Every displayed Pāli token uses the shared clickable PCED lookup. All 243 note references are superscript buttons linked to movable, non-dimming `註释` popups with the exact brown title bar, white title and cream body treatment used by 《止观法要》. The temporary proofreading panel and all generated English columns are absent. The reader uses the complete standard header, a fixed-height scrolling Contents panel without printed-book page numbers, 19px normal book text, a wide two-column reading area, standard Search and Book Mark controls, and the shared responsive reader assets. The Contents title uses the source 华文新魏 role and its entries use the source Hei role instead of a calligraphic Kai font. 序、序言、增订版序 and 凡例 use the same standard pale panel, top rule and 华文新魏 role as the primary chapter headings. Chapter and source subheadings are centred as in the printed book. All Hanyu-Pinyin font files and styles have been removed from this reader.

The landing page places the book under `玛欣德尊者译著 Dhamma Books authored or translated by Venerable Mahinda`, in the `戒律 The Code of Monastic Discipline` subcategory. The book card and the reader now use the separately stored actual cover file `bhikkhupatimokkha-cover.jpg`; Bhante Mahinda's personal seal is not used as the book cover. Static validation confirms four chapter sections, 74 source-faithful Contents destinations, no visible printed-page labels in the Contents or Chapter 2, 12,255 clickable Pāli spans, 243 superscript references and 243 corresponding note records, with no pinyin or generated English translation elements. Browser visual and interaction verification remains pending because a Chromium executable is unavailable in this environment. This is a manual-upload package; no push, PR, merge or deployment was performed.
...
The landing page loads the complete PCED base from `patisambhidamagga-pced-data.js`, applies `pced-standard-data.js`, and resolves through `pced-lookup-core.js`, matching the dictionary data and shared linguistic resolver used by reader word-clicks. It loads `pced-approved-terms.js` for Chinese-priority terminology, plus `pced-books-popup.css`, `pced-index-search.js` and `pced-popup-standard.js`; it does not contain a separate copy of the dictionary or modify individual book readers. Resolver v3.6.0, search module v1.4.0, and shared popup v1.5.0 add the compact **Inflections / 词形** panel for recognized noun and verb paradigms, separating verified whole-word mappings from generated possible regular forms. Resolver v3.6.0 also maps the verified absolutives `paṭisuṇitvā`, `paṭissuṇitvā`, and `paṭissutvā` to `paṭissuṇāti` and labels them “having agreed/promised,” including when PCED also contains the surface form as an entry. Search module v1.4.0 preserves the successful inflection analysis found while expanding an ASCII query into possible Pāli diacritic spellings, so `vihethessanti` displays `viheṭhessanti → viheṭheti` and its future-tense analysis instead of showing only the lemma entry. The popup retains its index-search, reader, movement, stacking, and non-dimming behavior.
