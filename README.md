...
The English reading panel now follows the supplied `TOW for Realization of Nibbā_na - pamc.docx` typography and layout: body and Unicode Pāli use Tahoma (Pāli italic), paragraphs retain the Word first-line and hanging-indent relationships, and headings retain the source small-cap character. The nine kamma-born kalāpas are restored as the Word source's two-column table on desktop, with aligned numbered continuations. On mobile the two source columns stack as full-width sections and long Pāli compounds wrap inside their cells, preventing overlap or document-level overflow even at an enlarged reader font. The Chinese panel, book text, Contents, language switching, notes, PCED terms, IDs and shared reader controls are unchanged. Structural validation confirms identical English text and reference counts before and after this formatting-only pass; deployed physical-device confirmation remains pending.
