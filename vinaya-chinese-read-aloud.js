(()=>{
  ...
  const paliScriptSelect=document.getElementById('read-aloud-pali-script');
  ...
  function populateVoices(){const saved=window.DhammaBooksReadAloudVoice?.load()||{voice:'__male__'},previous=voiceSelect.dataset.ready?voiceSelect.value:saved.voice,voices=synth.getVoices().filter(v=>/^zh/i.test(v.lang));voiceSelect.replaceChildren(new Option('优先使用男声（如有）','__male__'),new Option('自动选择',''),...voices.map(v=>new Option(`${v.name} (${v.lang})`,v.voiceURI)));voiceSelect.value=voices.some(v=>v.voiceURI===previous)||previous===''||previous==='__male__'?previous:'__male__';voiceSelect.dataset.ready='true';const pali=window.DhammaBooksReadAloudVoice,prior=paliVoiceSelect.dataset.ready?paliVoiceSelect.value:saved.paliVoice||'__indic__',options=pali.paliVoices(synth.getVoices());paliVoiceSelect.replaceChildren(new Option('优先印度语系声音（如有）','__indic__'),new Option('英语声音','__english__'),...options.map(v=>new Option(`${v.name} (${v.lang})`,v.voiceURI)));paliVoiceSelect.value=options.some(v=>v.voiceURI===prior)||prior==='__english__'?prior:'__indic__';paliVoiceSelect.dataset.ready='true';paliScriptSelect.value=saved.paliScript||'roman'}
  function speakChunk(runToken){if(runToken!==token)return;if(chunkIndex>=chunks.length){blockIndex++;return speakBlock(runToken)}const utterance=window.DhammaBooksReadAloudVoice.makeUtterance(chunks[chunkIndex++],rateSelect.value,synth.getVoices(),voiceSelect.value,paliVoiceSelect.value,paliScriptSelect.value);utterance.onend=()=>speakChunk(runToken);utterance.onerror=event=>{if(event.error!=='canceled'&&event.error!=='interrupted')finish('朗读发生错误，请再试一次。')};synth.speak(utterance)}
  function speakBlock(runToken){if(runToken!==token)return;if(blockIndex>=blocks.length)return finish();clearHighlight();const block=blocks[blockIndex];current=block.el;current.classList.add('read-aloud-current');current.scrollIntoView({behavior:'smooth',block:'center'});chunks=window.DhammaBooksReadAloudVoice.speechChunks(block.text);chunkIndex=0;status.textContent=`正在朗读第 ${blockIndex+1} 段，共 ${blocks.length} 段`;speakChunk(runToken)}
