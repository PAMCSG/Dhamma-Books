...
## 10A. Main-menu author subcategories

When an author category on `index.html` needs a language or subject subcategory, use the existing Venerable Mahinda subheading style for consistency. Put the `<h3 class="mahinda-subcategory">` in the same `.grid` as its book cards so it spans every grid column. Use the established rectangular `#EEE1CF` background, `#633919` text, 8 px radius, 9 px by 14 px padding and 18 px font (16 px on mobile). Place the local-language label first and the English label in `<span class="en">` with the existing Times New Roman treatment. Do not add an inline style or create a separate book grid for a subcategory. This also applies to Myanmar-language books under The Most Venerable Pa-Auk Tawya Sayadawgyi.

When a supplied first PDF page is a spread and the user identifies only one side as the cover, crop to that side rather than displaying the complete spread on the cover card or menu. Preserve the complete PDF separately when source comparison is needed.

For Unicode conversion from an old Burmese font, check the entire book for unattached combining marks and visibly stretched spacing. In particular, a reflowed Burmese paragraph must not inherit justified alignment that separates glyph clusters or creates exaggerated gaps. Preserve intentional verse line breaks and check the converted text against the visible source. Some PDFs simulate bold by overprinting the same line at small offsets; deduplicate those coincident extraction layers without removing distinct source lines. A zero count of unconverted Latin glyphs does not by itself establish correct Burmese wording.
...
- `aganna-anapanasati-difficulties-burmese.html` was created on 25 September 2026 from the supplied 32-page Burmese source PDF. The PDF is source material only; the public index opens the reflowable HTML reader. Page 1 supplies the cover, while printed page numbers, running heads and page-turn boundaries are omitted from the reading flow. The legacy Zawgyi source text is converted to standard Myanmar Unicode for device-independent display, Search and Azure Burmese speech. The introduction and all seven principal sections are present in a one-column reader with an eight-entry Burmese Contents panel. The complete initial header contains the logo, Burmese title, `မာတိကာ`, A−, A+, Read Aloud, one `စာညှပ်`, Search field and Search button, preventing header flash. Read Aloud uses the shared Burmese online voices with Microsoft Thiha as the default and Microsoft Nilar as the alternative. The book uses the standard width, cover card, heading hierarchy, mobile two-row sticky header and first-page cover, and is listed in a new Venerable Agañña category immediately after Chanting Book. Source, Unicode, structural, target, asset and JavaScript checks pass; deployed desktop and physical-mobile confirmation remains pending.

- `anapana-vipassana-burmese.html` is a conversion proofing draft of the supplied 87-page older-font PDF. The book is listed under Sayadawgyi in a Myanmar-language subcategory styled exactly like the Mahinda menu subheadings. Only the right half of the PDF's first page is used as its cover. The body uses left-aligned Unicode Burmese to avoid justified spacing that visually separates glyphs; opening verses retain separate source lines. Repeated extraction layers used by the PDF to make some lines bold were removed by comparing text and page coordinates. Its 32 Contents links, page coverage, assets and initial header structure were checked. The source PDF remains linked for comparison. Burmese wording, visual rendering and mobile interaction still require review before the draft can be marked final.
